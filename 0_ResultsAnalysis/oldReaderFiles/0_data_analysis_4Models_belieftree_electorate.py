...
for i in range(total_runs):
	...
	df_B.append(pd.read_csv('2_electorate_B_event2_' + str(i) + '.csv'))
	agent_trees_B.append([])

	df_Bplus.append(pd.read_csv('1_electorate_B+_event2_' + str(i) + '.csv'))
	agent_trees_Bplus.append([])

	df_3S.append(pd.read_csv('1_electorate_3S_event2_' + str(i) + '.csv'))
	agent_trees_3S.append([])

	df_ACF.append(pd.read_csv('1_electorate_ACF_event2_' + str(i) + '.csv'))
...
for file_number in range(total_runs):
	...
	for index, row in df_B[file_number].iterrows():
		...
		store2 = eval(store1)
		agent_trees_B[file_number][row['ElectorateID']].append(store2)
	for index, row in df_Bplus[file_number].iterrows():
		store1 = row['Belieftree']
		store2 = eval(store1)
		agent_trees_Bplus[file_number][row['ElectorateID']].append(store2)
	for index, row in df_3S[file_number].iterrows():
		store1 = row['Belieftree']
		store2 = eval(store1)
		agent_trees_3S[file_number][row['ElectorateID']].append(store2)
	for index, row in df_ACF[file_number].iterrows():
		store1 = row['Belieftree']
		store2 = eval(store1)
		agent_trees_ACF[file_number][row['ElectorateID']].append(store2)
...
axarr[0].legend()
f.savefig('Electorate_' + str(issue) + '_4Model_E0_00-29.png')
	...
	for k in range(3):
		for file_number in range(total_runs): # Going through each of the file one by one
			...
			for i in range(500):
				temp_aim.append(agent_trees[file_number][j][k][i][issue][1])
			if file_number == 0:
				axarr[j].plot(temp_aim, colour_range2[k], linewidth=1, label = label_names[k])
			else:
				axarr[j].plot(temp_aim, colour_range2[k], linewidth=1)
	if j == 0:
