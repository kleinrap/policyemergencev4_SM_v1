...
label_names = ['PM1', 'PM2', 'PM3', 'PM4', 'PM5', 'PM6']
...
for j in range(4):
	...
	for k in range(3):
		for file_number in range(total_runs): # Going through each of the file one by one
			...
			temp_aim = []
			for i in range(500):
				temp_state.append(agent_trees[file_number][j][k+6][i][issue][0])
				temp_aim.append(agent_trees[file_number][j][k+6][i][issue][1])
			if file_number == 0:
...
axarr[3].legend()
f.savefig('3PMs_' + str(issue) + '_4Model_E0_00-29.png')
...
for j in range(4):
	for k in range(3):
		for file_number in range(total_runs): # Going through each of the file one by one
			temp_CR = []
			for i in range(500):
				temp_CR.append(agent_trees[file_number][j][k+6][i][issue][0])
			if file_number == 0:
