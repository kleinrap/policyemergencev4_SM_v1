...
class RandomActivationByBreed(RandomActivation):
    ...
    def add(self, agent):
        ...
        # for length in sorted(self.agents_by_breed):
        #     print(length, [t.originalWord for t in self.agents_by_breed[length]])

    def __str__(self):
        ret_val = ''
        items = self.agents_by_breed.items()
        for itemKey, policymaker_data in items:
            ret_val += '['
            for itemValue in policymaker_data:
                ret_val += str(itemValue)
                ret_val += ", "
            ret_val = ret_val[:-2]
            ret_val += ']'
        return ret_val
